...
import (
	...
	timeutils "github.com/chengfeiZhou/notifyTool/pkg/time"
...
func DefaultLogger() Logger {
	...
	if err != nil {
		log.Printf("create log error: %s, use Default", err.Error())
		lg = zap.NewExample()
	}
	return &AppLogger{slg: lg.Sugar(), lg: lg, level: zap.NewAtomicLevelAt(zapcore.Level(DebugLevel))}
}
...
func NewLogger(opts ...Option) (Logger, error) {
	cfg := zap.Config{
		...
		EncoderConfig: zapcore.EncoderConfig{
			...
			EncodeTime:     zapcore.TimeEncoderOfLayout(timeutils.TimeFormatMS), // 时间格式
